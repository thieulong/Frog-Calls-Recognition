...
class QueryDisplay:
  ...
  def update_audio(self):
    ...
    if self.audio is None:
      self.audio = np.array(audio_io.load_audio(self.uri, self.sample_rate_hz))
...
class EmbeddingDisplayGroup:
  ...
  def iterator_with_audio(
    ...
    audio_iter_ = audio_io.multi_load_audio_window(
...
def pcen_melspec_display(
    audio: np.ndarray, sample_rate_hz: int, root: float = 4.0
):
  """Compute a PCEN mel spectrogram for display."""
...
def get_melspec_layer(
  ...
  return lambda x: pcen_melspec_display(
      x, sample_rate_hz=sample_rate_hz, root=root
  )
